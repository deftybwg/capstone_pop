...
**5. Run `python3 manage.py makemigrations` then `python3 manage.py migrate`, and you should be good to go. You may need to make a file called .env in the myfitness app folder and generate your private SECRET_KEY with Django, or you can try skipping this step. So may need to query Google or ask ChatGPT about how to add a .env file with a DB username:password. Once you've added your secret key, you should be able to move to step 6, but it may throw and error about migrations again so try running the first two commands in this step.**<br>
